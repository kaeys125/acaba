...
-- Executar: sqlite3 limpeza.db < add-performance-indices.sql

-- Índices em bookings (tabela mais consultada)
-- Nota: idx_bookings_user_id, idx_bookings_date, idx_bookings_status já existem
CREATE INDEX IF NOT EXISTS idx_bookings_user_service ON bookings(user_id, service_id);
CREATE INDEX IF NOT EXISTS idx_bookings_booking_date_status ON bookings(booking_date, status);
